...
public class WheelView extends View {
    ...
    public final void setAdapter(WheelAdapter adapter) {
        if(adapter instanceof NumericWheelAdapter) {
            minValue = ((NumericWheelAdapter)adapter).getMinValue();
        }
    ...
    public int getItemPosition(int value){
        if(adapter instanceof NumericWheelAdapter) {
            return ((NumericWheelAdapter)adapter).getItemPosition(value);
        }
        return 0;
    }
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    Log.d("BTT","selectedItem:"+selectedItem+" contentText:"+contentText);
